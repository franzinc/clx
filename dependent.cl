...
(defun open-x-stream (host display protocol)
  ...
  (let ((fd (connect-to-server
	     (excl:ics-target-case
	      (:-ics (string host))
	      (:+ics (excl:string-to-euc (string host))))
	     display)))
    (when (minusp fd)
      (error "Failed to connect to server: ~A ~D" host display))
    fd))
...
(deftype pixarray-2-element-type ()
  '(unsigned-byte 2))
...
(deftype pixarray ()
  '(or pixarray-1 pixarray-2 pixarray-4 pixarray-8
    pixarray-16 pixarray-24 pixarray-32))
...
(defmacro with-underlying-simple-vector
	  ((variable element-type pixarray) &body body)
  `(let ((,variable (cdr (excl::ah-data ,pixarray))))
     (declare (type (simple-array ,element-type (*)) ,variable))
     ,@body))

;;; These are used to read and write pixels from and to CARD8s.

;;; READ-IMAGE-LOAD-BYTE is used to extract 1, 2 and 4 bit pixels from CARD8s.
...
(defun fast-read-pixarray-1 (buffer-bbuf index array x y width height
  ...
  (declare (type buffer-bytes buffer-bbuf)
	   ...
	   (type (member 1 2 4 8 16 24 32) bits-per-pixel)
...
;; >>> This derived by interpolating fast-read-pixarray-1 & -4,
;; >>> with only a fuzzy idea of what's really happening.
;; >>> "quarter" used here to mean half a nibble (two bits)
#+(or lcl3.0 excl)
(defun fast-read-pixarray-2 (buffer-bbuf index array x y width height
			     padded-bytes-per-line bits-per-pixel)
  (declare (type buffer-bytes buffer-bbuf)
	   (type pixarray-2 array)
	   (type card16 x y width height)
	   (type array-index index padded-bytes-per-line)
	   (type (member 1 2 4 8 16 24 32) bits-per-pixel)
	   (ignore bits-per-pixel))
  ...
  (with-vector (buffer-bbuf buffer-bytes)
    (with-underlying-simple-vector (vector pixarray-1-element-type array)
      (do* ((start (index+ index
		   ...
		   (index+ start padded-bytes-per-line))
	    (y 0 (index1+ y))
	    (left-quarters (index-mod (index- x) 4))
	    (right-quarters (index-mod (index- width left-quarters) 4))
	    (middle-quarters (index- width left-quarters right-quarters))
	    (middle-bytes (index-floor middle-quarters 4)))
	   ((index>= y height))
	(declare (type array-index start y
		       left-quarters right-quarters middle-quarters
		       middle-bytes))
	(cond ((index< middle-quarters 0)
	       (let ((byte (aref buffer-bbuf (index1- start)))
		     (x (array-row-major-index array y left-quarters)))
		 (declare (type card8 byte)
			  (type array-index x))
		 (when (index> right-quarters 2)
		   (setf (aref vector (index- x 1))
			 (read-image-load-byte 1 3 byte)))
		 (when (and (index> left-quarters 1)
			    (index> right-quarters 1))
		   (setf (aref vector (index- x 2))
			 (read-image-load-byte 1 2 byte)))
		 (when (index> left-quarters 2)
		   (setf (aref vector (index- x 3))
			 (read-image-load-byte 1 1 byte)))))
	      (t
	       (unless (index-zerop left-quarters)
		 (let ((byte (aref buffer-bbuf (index1- start)))
		       (x (array-row-major-index array y left-quarters)))
		   (declare (type card8 byte)
			    (type array-index x))
	       ...
	       (do* ((end (index+ start middle-bytes))
			...
			(index+ x 4)))
		    ((index>= i end)
		     (unless (index-zerop right-quarters)
		       (let ((byte (aref buffer-bbuf end))
			     (x (array-row-major-index
				 array y
				 (index+ left-quarters middle-quarters))))
			 (declare (type card8 byte)
				  (type array-index x))
			 (setf (aref vector (index+ x 0))
			       (read-image-load-byte 2 0 byte))
			 (when (index> right-quarters 1)
			   (setf (aref vector (index+ x 1))
				 (read-image-load-byte 2 2 byte))
			   (when (index> right-quarters 2)
			     (setf (aref vector (index+ x 2))
				   (read-image-load-byte 2 4 byte))
				     )))))
		 (declare (type array-index end i x))
		 (let ((byte (aref buffer-bbuf i)))
		   (declare (type card8 byte))
...
(defun fast-read-pixarray (bbuf boffset pixarray
  ...
  (or
    ...
    (let ((function
		...
		#+(or lcl3.0 excl)
		(and (index= bits-per-pixel 2)
		     #'fast-read-pixarray-2)
		#+(or lcl3.0 excl)
...
;; >>> This derived by interpolating fast-write-pixarray-1 & -4,
;; >>> with only a fuzzy idea of what's really happening.
;; >>> "quarter" used here to mean half a nibble (two bits)
#+(or lcl3.0 excl)
(defun fast-write-pixarray-2 (buffer-bbuf index array x y width height
			      padded-bytes-per-line bits-per-pixel)
  (declare (type buffer-bytes buffer-bbuf)
	   (type pixarray-2 array)
	   (type card16 x y width height)
  ...
  #.(declare-buffun)
  (with-vector (buffer-bbuf buffer-bytes)
    (with-underlying-simple-vector (vector pixarray-2-element-type array)
      (do* ((h 0 (index1+ h))
	    (y y (index1+ y))
	    (right-bits (index-mod width 4))
	    (middle-bits (index- width right-bits))
	    (middle-bytes (index-ceiling middle-bits 4))
	    (start index (index+ start padded-bytes-per-line)))
	   ((index>= h height))
	(declare (type array-index h y right-bits middle-bits
		       middle-bytes start))
	(do* ((end (index+ start middle-bytes))
	      (i start (index1+ i))
	      (start-x x)
	      (x (array-row-major-index array y start-x) (index+ x 4)))
	     ((index>= i end)
	      (unless (index-zerop right-bits)
		(let ((x (array-row-major-index
			   array y (index+ start-x middle-bits))))
...
(defun fast-copy-pixarray (pixarray copy x y width height bits-per-pixel)
  ...
  (or
	...
	(2  (copy pixarray-2  pixarray-2-element-type))
